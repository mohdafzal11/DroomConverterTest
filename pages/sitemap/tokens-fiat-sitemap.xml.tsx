...
const SITEMAP_CACHE_KEY = 'sitemap_tokens_fiat_index_xml_cache';
...
export const getServerSideProps: GetServerSideProps = async ({ res }) => {
  try {
    ...
    if (cachedSitemap) {
      console.log('Serving tokens-fiat sitemap index from Redis cache');
    ...
    console.log('Generating fresh tokens-fiat sitemap index and caching in Redis');
    ...
    // Get number of fiat currencies
    const fiatCurrencies = Object.keys(CURRENCIES);
    
    // Calculate total number of URLs (each token to each fiat)
    const totalUrls = tokenCount * fiatCurrencies.length;
    ...
    for (let i = 1; i <= sitemapCount; i++) {
      sitemapEntries.push(`
        <sitemap>
          <loc>${escapeXml(`${domain}/sitemap/tokens-fiat/${i}.xml`)}</loc>
    ...
    console.error('Error generating tokens-fiat sitemap index:', error);
    return {
      props: {},
    };
  }
...
const TokensFiatSitemapIndex = () => null;
export default TokensFiatSitemapIndex;
